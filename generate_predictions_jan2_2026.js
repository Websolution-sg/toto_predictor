...
function enhancedFrequencyCompatibility(draws) {
    ...
    return selectWithBalance(candidates).sort((a, b) => a - b);
}
...
function patternAnalysisAdvanced(draws) {
    ...
    for (let attempt = 0; attempt < 500 && candidates.length < 20; attempt++) {
        ...
        if (nums.length === 6) {
            ...
            const score = 100 - Math.abs(sum - targetSum) - Math.abs(range - avgRange) * 0.5;
            
            // Pattern validation boost
            let patternBoost = 0;
            if (nums.some(n => [22, 24, 30].includes(n))) patternBoost = 15;
            const finalScore = score + patternBoost;
            
            candidates.push({ nums: nums.sort((a, b) => a - b), score: finalScore });
        }
    }
